...
# Script to analyze CentOS Stream RPM packages for downstream patch files
# Checks for .patch files in each repository
...
OUTPUT_FILE="patch_files_packages.csv"
TEMP_DIR=$(mktemp -d)
...
# Count .patch files in a project
check_patch_files() {
    local project_id=$1
    ...
    # Count .patch files
    local patch_count
    patch_count=$(echo "$files" | jq '[.[] | select(.name | endswith(".patch"))] | length' 2>/dev/null || echo "0")
...
analyze_packages() {
    ...
    echo "Analyzing $total_projects packages for downstream patch files..."
    echo "=========================================="
    
    # Initialize results with CSV header
    echo "package_name,project_id,web_url,patch_count" > "$RESULTS_FILE"
    
    local current=0
    local patch_files_count=0
    ...
    while IFS= read -r project; do
        ...
        local patch_count
        patch_count=$(check_patch_files "$project_id" "$project_name")
        
        if [ "$patch_count" -gt 0 ]; then
            echo "✓ Has $patch_count patch files"
            ((patch_files_count++))
            
            # Add to results (CSV format)
            echo "$project_name,$project_id,$project_url,$patch_count" >> "$RESULTS_FILE"
        else
            echo "✗ No patch files detected"
        fi
    ...
    output_results "$total_projects" "$patch_files_count"
}

# Output the analysis results
output_results() {
    local total_projects=$1
    local patch_files_count=$2
    ...
    echo "Packages with patch files: $patch_files_count"
    
    if [ "$total_projects" -gt 0 ]; then
        local percentage
        percentage=$(echo "scale=1; $patch_files_count * 100 / $total_projects" | bc -l)
        echo "Percentage: $percentage%"
    fi
    
    if [ "$patch_files_count" -gt 0 ]; then
        echo
        echo "Packages with patch files:"
        echo "----------------------------------------"
        
        # Skip header line and display results
        tail -n +2 "$RESULTS_FILE" | while IFS=',' read -r name id url patch_count; do
            echo "• $name ($patch_count patch files)"
            echo "  URL: $url"
        done
    fi
    
    # Create final CSV output file with metadata header
    local timestamp
    timestamp=$(date '+%Y-%m-%d %H:%M:%S')
    
    {
        echo "# CentOS Stream RPM Package Patch Files Analysis"
